...
char pokemon(int n, int m, char a[][100], char you, int energy) {
    int x=0,y=0,x1,y1,i,j,x2,y2,t=75,k;
    char move;
    ...
    for (k=1; k<7; k++) {
            ...
            if(a[x-k][y] == '#' || energy<(a[x-k][y]-47) || (a[x-k][y]>=97 && a[x-k][y]<=122) || x-k>=n || x-k<=0) no=no+(57/k);
                else if(a[x-k][y]>47 && a[x-k][y]<=57) no=no-((t-a[x-k][y]));
                    else if(a[x-k][y] == ' ') no=no+k;
                        else no--;
            if(a[x][y+k] == '#' || energy<(a[x][y+k]-47) || (a[x][y+k]>=97 && a[x][y+k]<=122) || y+k>=m || y+k<=0)  e=e+(57/k);
                else if(a[x][y+k]>47 && a[x][y+k]<=57) e=e-((t-a[x][y+k]));
                    else if(a[x][y+k] == ' ') e=e+k;
                        else e--;
            if(a[x][y-k] == '#' || energy<(a[x][y-k]-47) || (a[x][y-k]>=97 && a[x][y-k]<=122) || y-k>=m || y-k<=0)  w=w+(57/k);
                else if(a[x][y-k]>47 && a[x-k][y]<=57) w=w-((t-a[x-k][y]));
                    else if(a[x][y-k] == ' ') w=w+k;
                        else w--;
            if(a[x+k][y] == '#' || energy<(a[x+k][y]-47) || (a[x+k][y]>=97 && a[x+k][y]<=122) || x+k>=n || x+k<=0)  s=s+(57/k);
                else if(a[x+k][y]>47 && a[x+k][y]<=57) s=s-((t-a[x+k][y]));
                    else if(a[x+k][y] == ' ') s=s+k;
                        else s--;

    }
    if (a[x-1][y]>47 && a[x-1][y]<=57) no=no-(t-a[x-1][y]);
    if (a[x][y+1]>47 && a[x][y+1]<=57) e=e-(t-a[x][y+1]);
    if (a[x][y-1]>47 && a[x][y-1]<=57) w=w-(t-a[x][y-1]);
    if (a[x+1][y]>47 && a[x+1][y]<=57) s=s-(t-a[x+1][y]);
    ...
    if ((x+1)>m) s=100000;
    if (no<w && no<s && s==no)
    if(a[x-1][y]>47 && a[x-1][y]<=57) no=no-(t-a[x-1][y])-12;
    else no++;
    if(a[x-2][y]>47 && a[x-2][y]<=57) no=no-(t-a[x-2][y]-10);
    else no++;
    if (w<no && w<e && w==s)
    if(a[x+1][y]>47 && a[x+1][y]<=57) s=s-(t-a[x+1][y]-12);
    else s++;
    if(a[x+2][y]>47 && a[x+2][y]<=57) s=s-(t-a[x+2][y]-10);
    else s++;
    if (s<no && s<w && s==e) e++;
    if (e<w && e<s && e==no)
    if(a[x][y+1]>47 && a[x][y+1]<=57) e=e-(t-a[x][y+1]-12);
    else e++;
    if(a[x][y+2]>47 && a[x][y+2]<=57) e=e-(t-a[x][y+2]-10);
    else e++;
    if (w<s && w<e && w==no)
    if(a[x][y-1]>47 && a[x][y-1]<=57) w=w-(t-a[x][y-1]-12);
    else w++;
    if(a[x][y-2]>47 && a[x][y-2]<=57) w=w-(t-a[x][y-2]-10);
    else w++;
    if (w<s && w<no && w==e) w++;
